...
class ResponseFormatAi(BaseModel):
    reasoning: str = Field(
        description="Скрытый этап. 1) Анализ динамики (лучше/хуже/в норме). 2) Выбор факта из <RAG_CONTEXT> (механизм). 3) Проверка History (не повторять выражения, смысл! И не повторять миссии/ритуалы). 4) Формирование гипотезы."
    )
    ...
    response: str = Field(
        description="60-80 токенов. Основной совет. Структура: 'Инсайт из <RAG_CONTEXT> (почему это так)' -> 'Рекомендация (что сделать) из <RAG_CONTEXT>'. Подробная логика описана в system"
    )
